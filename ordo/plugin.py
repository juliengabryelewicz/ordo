...
def find_required_plugin(plugin_directory):
    infofile_path = os.path.join(plugin_directory, PLUGIN_INFO_FILENAME)
    cp = parse_info_file(infofile_path)
    return cp.get('Plugin', 'Required')
...
class PluginList:
    ...
    def parse_plugin(self, plugin_directory):
        ...
        cp = parse_info_file(infofile_path)
        plugin_class = get_plugin_class(cp.get('Plugin', 'Id'),plugin_directory)
...
class PluginInfo(object):
    ...
    def required_modules(self):
        return self._cp.get('Plugin', 'Required')
